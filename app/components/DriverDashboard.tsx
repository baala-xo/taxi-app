      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        ...
        <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <button onClick={handleAvailabilityToggle} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${isAvailable ? 'bg-green-500' : 'bg-gray-400'}`}><span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${isAvailable ? 'translate-x-6' : 'translate-x-1'}`}/></button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12">
        ...
        <div className="mt-4 space-y-4">
          {currentBookings.length > 0 ? currentBookings.map((ride) => (
            <div key={ride.id} className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-between">
              <div>
                <p><b>From:</b> {ride.pickup_location} <b>To:</b> {ride.dropoff_location}</p>
                <p className="text-lg font-bold">${ride.price}</p>
              </div>
              <button onClick={() => handleCompleteRide(ride.id)} className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded">Mark as Complete</button>
            </div>
          )) : <div className="p-8 text-center bg-gray-100 dark:bg-gray-800 rounded-lg"><p>You have no active bookings.</p></div>}
        </div>
      </div>
           ...
           {completedRides.length > 0 ? completedRides.map((ride) => (
            <div key={ride.id} className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg">
              ...
              <div className="mt-2 pt-2 border-t flex justify-between items-center">
                ...
                <div>
                  <p className="font-semibold text-right">{ride.rating ? `${ride.rating} ★` : 'Not rated'}</p>
